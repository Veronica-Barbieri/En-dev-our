...
void QtSalariesController::calcFullSal(std::vector<std::pair<int, int>> stats) {
    try {
        ...
        std::string diffs = std::to_string(model->getDiffSal());
...
void QtSalariesController::getPayrollFromFile(QJsonObject root){
    ...
    int twh = root.value("tot_worked_hours").toInt();
    int hwh = root.value("highest_worked_hours").toInt();
    double hsen = root.value("highest_seniority").toDouble();
    double diffs = root.value("diff_salary").toDouble();
    double diffh = root.value("diff_hours").toDouble();
    ...
    view->updatePayrollInfo(QString::number(tbs).toStdString(),
                      ...
                      QString::number(diffs).toStdString(),
                      QString::number(diffh).toStdString());
}
...
QJsonObject QtSalariesController::writePayrollToFile(){
    ...
    QJsonValue diffs = model->getDiffSal();
    QJsonValue diffh = model->getDiffHours();
    ...
    json_payroll.insert(QString("diff_salary"), diffs);
    json_payroll.insert(QString("diff_hours"), diffh);
